  ...
  async allocations(status: AllocationStatus): Promise<Allocation[]> {
    try {
      ...
      if (
        ...
        this.logger.warn(
          `No ${AllocationStatus[status]} allocations found for indexer '${this.indexer}'`,
        )
        return []
      }
  ...
  async resolvePOI(
    ...
    switch (force) {
          ...
          case false:
            return (
              (await this.indexingStatusResolver.proofOfIndexing(
                ...
                await this.transactionManager.ethereum.getBlock(
      ...
      case false: {
        ...
        const epochStartBlock = await this.transactionManager.ethereum.getBlock(
